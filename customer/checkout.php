...
// Inisialisasi variabel untuk Kupon/Voucher (untuk tampilan awal)
// Anda harus menambahkan logika di 'proses/get_checkout.php' atau file terpisah untuk menyimpan/mengambil kode kupon dan diskon yang berlaku
$coupon_code = '';
$voucher_status_message = '';
?>
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <style>
        ...
        .btn-pink-primary:hover {
            background-color: #c21a6d;
            border-color: #c21a6d;
            color: white;
        }

        /* CSS BARU: Style untuk tombol kupon */
        .btn-outline-pink-primary {
            color: #d63384;
            border-color: #d63384;
        }

        .btn-outline-pink-primary:hover {
            background-color: #d63384;
            color: white;
        }
...
<body>
    ...
    <div class="container my-5">
        ...
        elseif (empty($cart_items)):
            ...
            <div class="alert alert-warning" role="alert">
                <i class="fas fa-shopping-cart me-2"></i> Keranjang belanja Anda **kosong**. Silakan kembali ke halaman
                produk.
            </div>
        <?php endif; ?>

        <form method="POST" id="checkoutForm">
            <input type="hidden" id="applied_coupon_code" name="applied_voucher_code" value="">

            <div class="row g-4">
                <div class="col-lg-7">
                    <div class="card shadow-sm mb-4 checkout-summary-card">
                        <div class="card-header bg-white">
                            <h5 class="card-title mb-0 text-pink-primary"><i class="fas fa-map-marker-alt me-2"></i>
                        ...
                        <div class="card-body">
                            <?php if (!empty($user_addresses)): ?>
                                <?php foreach ($user_addresses as $address): ?>
                                    <div
                                        class="form-check p-3 mb-2 border rounded <?php echo ($selected_address['id'] == $address['id'] ? 'border-pink-primary bg-light' : ''); ?>">
                                        <input class="form-check-input" type="radio" name="address_id"
                                            id="address_<?php echo $address['id']; ?>" value="<?php echo $address['id']; ?>"
                                            <?php echo ($selected_address && $selected_address['id'] == $address['id'] ? 'checked' : ''); ?> required>
                                        <label class="form-check-label w-100" for="address_<?php echo $address['id']; ?>">
                                            <strong class="d-block"><?php echo htmlspecialchars($address['recipient_name']); ?>
                                                - (<?php echo htmlspecialchars($address['phone_number']); ?>)</strong>
                                            <small class="d-block text-muted"><?php echo htmlspecialchars($address['label']); ?>
                                                <?php echo ($address['is_active'] ? '<span class="badge bg-secondary">Default</span>' : ''); ?></small>
                                            <p class="mb-0 small text-wrap">
                                                <?php echo htmlspecialchars($address['full_address'] . ', ' . $address['city'] . ', ' . $address['postal_code']); ?>
                                            </p>
                                        </label>
                                    </div>
                                <?php endforeach; ?>
                                <small class="text-muted"><a href="profile.php?tab=address"
                                        class="text-pink-primary text-decoration-none"><i
                                            class="fas fa-plus-circle me-1"></i> Tambah Alamat Baru</a></small>
                            <?php else: ?>
                                <div class="alert alert-info">
                                    <i class="fas fa-exclamation-circle me-2"></i> Anda belum memiliki alamat. <a
                                        href="profile.php?tab=address" class="alert-link text-pink-primary">Tambahkan
                                        sekarang.</a>
                    ...
                    <div class="card shadow-sm mb-4 checkout-summary-card">
                        <div class="card-header bg-white">
                            <h5 class="card-title mb-0 text-pink-primary"><i class="fas fa-credit-card me-2"></i> Metode
                                Pembayaran</h5>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="mb-3 border rounded p-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment_method" id="payment_qris"
                                        value="QRIS" data-bs-toggle="collapse" data-bs-target="#collapseQris"
                                        aria-expanded="true" aria-controls="collapseQris" checked required>
                                ...
                                <div class="collapse show mt-2" id="collapseQris" data-bs-parent="#paymentOptions">
                                    <p class="text-muted small">Scan kode di bawah menggunakan aplikasi pembayaran Anda:
                                    </p>
                                    <div class="qris-container">
                                        <img src="../assets/img/qris.jpeg" class="img-fluid w-100 card-bayar"
                            ...
                            <div class="mb-3 border rounded p-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment_method" id="payment_tf"
                                        value="Transfer Bank" data-bs-toggle="collapse"
                                        data-bs-target="#collapseTransfer" aria-expanded="false"
                                        aria-controls="collapseTransfer" required>
                            ...
                            <div class="mb-3 border rounded p-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="radio" name="payment_method"
                                        id="payment_ewallet" value="E-Wallet" data-bs-toggle="collapse"
                                        data-bs-target="#collapseEwallet" aria-expanded="false"
                                        aria-controls="collapseEwallet" required>
                ...
                <div class="col-lg-5">
                    <div class="card shadow checkout-summary-card sticky-top">
                        <div class="card-header bg-white">
                            <h5 class="card-title mb-0 text-pink-primary"><i class="fas fa-list-alt me-2"></i> Ringkasan
                                Pesanan</h5>
                        </div>
                        <div class="card-body">

                            <div class="mb-4 pb-2 border-bottom">
                                <h6 class="fw-bold mb-2 text-pink-primary"><i class="fas fa-tags me-1"></i> Punya Kode
                                    Kupon?</h6>
                                <div class="input-group">
                                    <input type="text" class="form-control" id="coupon_code"
                                        placeholder="Masukkan kode kupon"
                                        value="<?php echo htmlspecialchars($coupon_code); ?>"
                                        aria-label="Kode Kupon" aria-describedby="applyVoucherBtn">
                                    <button class="btn btn-outline-pink-primary" type="button"
                                        id="applyVoucherBtn">Terapkan</button>
                                </div>
                                <small id="voucher_status" class="form-text mt-1 text-muted">
                                    <?php echo empty($voucher_status_message) ? 'Masukkan kode untuk mendapatkan diskon.' : $voucher_status_message; ?>
                                </small>
                            </div>
                                    ...
                                    foreach ($cart_items as $item):
                                        ...
                                        <div class="d-flex mb-2 align-items-center">
                                            ...
                                            <div class="flex-grow-1">
                                                ...
                                                <small class="text-muted"><?php echo format_rupiah($item['price']); ?> x
                                                    <?php echo $item['quantity']; ?></small>
                                            </div>
                                            <span
                                                class="small fw-bold text-pink-primary"><?php echo format_rupiah($item['subtotal']); ?></span>
                                        </div>
                                    <?php
                                    endforeach;
                                else:
                            ...
                            <div class="summary-item mb-3">
                                ...
                                <span class="text-danger" id="discount_display">-
                                    <?php echo format_rupiah($discount_amount); ?></span>
    ...
    <script>
        ...
        function calculateTotal() {
            ...
            // Ambil nilai diskon yang sudah di-update oleh fungsi applyVoucher
        ...
        // ==========================================================
        // FUNGSI BARU: Logic untuk Voucher (Simulasi AJAX ke server)
        // ==========================================================
        function applyVoucher(voucherCode) {
            // Mock server response delay
            return new Promise(resolve => {
                setTimeout(() => {
                    const statusElement = document.getElementById('voucher_status');
                    ...
                    const appliedVoucherInput = document.getElementById('applied_coupon_code');

                    // Logika Mocking Kupon: "DISKON50" akan memberikan diskon Rp 50.000
                    if (voucherCode.toUpperCase() === 'DISKON50') {
                        const discountAmount = 50000; // Diskon tetap Rp 50.000

                        // 1. Update nilai diskon
                        discountValueElement.value = discountAmount;
                        discountDisplayElement.innerText = '- ' + formatRupiahJs(discountAmount);

                        // 2. Update status pesan
                        statusElement.innerText = `Kode Kupon ${voucherCode} berhasil diterapkan! Diskon: ${formatRupiahJs(discountAmount)}`;
                        statusElement.classList.remove('text-danger', 'text-muted');
                        statusElement.classList.add('text-success');

                        // 3. Simpan kode kupon di hidden input untuk proses checkout PHP
                        appliedVoucherInput.value = voucherCode;

                        resolve(true); // Success
                    } else if (voucherCode.trim() === '') {
                        // Reset jika input kosong
                        discountValueElement.value = 0;
                        discountDisplayElement.innerText = '- ' + formatRupiahJs(0);

                        statusElement.innerText = 'Masukkan kode untuk mendapatkan diskon.';
                        ...
                        statusElement.innerText = `Kode Kupon "${voucherCode}" tidak valid atau sudah kadaluarsa.`;
                        statusElement.classList.remove('text-success', 'text-muted');
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ==========================================================
            // NEW LOGIC: Event Listener untuk Tombol Kupon
            // ==========================================================
            const applyVoucherBtn = document.getElementById('applyVoucherBtn');
            const voucherInput = document.getElementById('coupon_code');
            ...
            if (applyVoucherBtn && voucherInput) {
                applyVoucherBtn.addEventListener('click', async function () {
                    ...
                    applyVoucherBtn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Memeriksa...';

                    await applyVoucher(voucherCode);

                    // Recalculate total setelah voucher diterapkan
                    calculateTotal();

                    // Kembalikan state tombol
                    applyVoucherBtn.disabled = false;
                    applyVoucherBtn.innerHTML = 'Terapkan';
                });

                // Opsi: Terapkan kupon saat tombol Enter ditekan
